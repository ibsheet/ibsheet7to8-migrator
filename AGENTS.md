...
6. **문자열 치환 시 인코딩 주의.** 한글 포함 소스는 Python `read_text/write_text(encoding='utf-8')` 또는 에디터 도구로 처리. (PowerShell `Get-Content`는 CP949 오독으로 한글 파괴 가능 → 금지)

---

## 설계 원칙: 결정론 80% + 판단 20% + 자기검증
...
| 메서드 | `CreatePivotTable`/`ShowPivotTable` → `makePivotTable` | 호출은 1:1이지만 **피벗 시트 id 규칙이 바뀐다**(v7 `원본id_Pivot` → v8 `pivotSheet_원본id`) → id 참조처를 함께 수정 |
