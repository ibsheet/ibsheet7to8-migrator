...
from srcio import read_source, write_output          # noqa: E402  (인코딩은 srcio 한 곳)
...
def detect_format_keywords(text, table):
    """v7 `Format:"키워드"` 를 찾아 v8 대응을 알린다 (자동으로 고치지 않는다).

    왜 자동 변환하지 않나
      대응이 **네 갈래**라 한 가지 치환으로 끝나지 않는다 —
      날짜는 `Format` 을 지우고 `Extend` 를 넣어야 하고, 마스킹은 `CustomFormat` 으로 옮기고,
      `Null*` 은 `CanEmpty:1` 을 **추가**해야 하며, 숫자형은 **지우는 것이 맞다.**
      키를 바꾸는 게 아니라 **줄의 구조를 바꾸는 일**이라 사람이 본다.

    왜 필요한가 (2026-08-12)
      v8 은 `Format:"Ymd"` 를 모른다. 그런데 변환도 플래그도 없이 **그대로 통과**하고 있었다.
      검증 3종은 전부 PASS 다 — 문법도 함수 파리티도 멀쩡하기 때문이다.
      결과는 날짜 서식·서버 전송값 포맷(`DataFormat`)이 **조용히 사라지는** 것이다.
      실측으로 `Ymd` 만 464개 화면에서 쓰였다.

    ★대소문자를 구분하지 않는다 — 고객 코드에 `ymd` 처럼 소문자로 쓴 화면이 있다.
    """
    keys = {k: v for k, v in (table or {}).items()
            if not k.startswith("_") and isinstance(v, dict)}
    if not keys:
        return []
    found = []
    lines = text.splitlines()
    # `(?<![\w$])` 가 없으면 `CustomFormat`·`DataFormat`·`CountFormat` 에도 걸린다
    pat = re.compile(r"(?<![\w$])Format\s*:\s*([\"'])([A-Za-z][A-Za-z0-9_]*)\1")
    lower = {k.lower(): k for k in keys}
    # v8 패턴 문자열(`yyyyMMdd`·`yyyyMM`)은 같은 글자가 반복된다. 키워드는 그렇지 않다.
    # 글자 종류만으로 가르면 `Ymd` 같은 키워드까지 패턴으로 흘려보낸다(실제로 그랬다).
    patternish = re.compile(r"([yYmMdDhHsS])\1")
    for m in pat.finditer(text):
        raw = m.group(2)
        name = lower.get(raw.lower())
        ln0 = text.count("\n", 0, m.start()) + 1
        if not name:
            # ★표에 없는 키워드도 흘려보내지 않는다. v8 이 모르는 값은 **무시되어**
            #   서식이 조용히 사라진다. 실측에 `email` 처럼 대응 없는 값이 나온다.
            if patternish.search(raw):
                continue                      # v8 패턴 문자열 — 그대로 두면 된다
            found.append({
                "rule": "FORMAT_UNKNOWN",
                "line": ln0,
                "desc": ("v7 `Format:\"%s\"` — **대응표에 없는 키워드다.** v8 이 모르는 값은 "
                         "오류 없이 **무시되어 서식이 사라진다.** `ibsheet-common.js` 의 "
                         "`IB_Preset` 에도 없는지 확인하고, 없으면 `Type`·`Format` 을 "
                         "직접 지정하거나 벤더에 확인한다." % raw),
                "code": (lines[ln0 - 1].strip()[:200] if 0 < ln0 <= len(lines) else ""),
            })
            continue
        info = keys[name]
        kind, v8 = info.get("kind"), info.get("v8") or ""
        if kind == "drop":
            head = "v7 `Format:\"%s\"` → **옮기지 않는다.**" % raw
        elif kind == "unknown":
            head = "v7 `Format:\"%s\"` → **대응이 없다.**" % raw
        else:
            head = "v7 `Format:\"%s\"` → `%s`. **`Format` 은 지운다.**" % (raw, v8)
        parts = [head]
        if kind == "preset_canempty":
            parts.append("★`CanEmpty:1` 을 **같이 넣어야 한다.**")
        if raw != name:
            parts.append("(원본이 `%s` 로 쓰여 있다 — v7 은 받아 줬다)" % raw)
        if info.get("note"):
            parts.append(info["note"])
        ln = text.count("\n", 0, m.start()) + 1
        found.append({
            "rule": "FORMAT_" + name.upper(),
            "line": ln,
            "desc": " ".join(parts),
            "code": (lines[ln - 1].strip()[:200] if 0 < ln <= len(lines) else ""),
        })
...
def main():
    ...
    rules = load_rules()
    original, src_enc = read_source(args.input)
    ...
    flags += detect_format_keywords(text, rules.get("v7_format_keywords"))
