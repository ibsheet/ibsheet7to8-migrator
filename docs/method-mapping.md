...
| `GetRowBackColorD(), SetRowBackColorD()` | (CSS `.IBColorDeleted`) | ⚠️ | v8 에 **메서드가 없다.** css 파일에서 `.IBColorDeleted` 클래스를 고친다(근거 v8 `migration/migration-04.md`). ★**실행 중 색을 바꾸던 코드는 그대로 옮길 수 없다** — CSS 는 정적이다 (`ROWBACKCOLOR_STATUS`) |
| `GetRowBackColorI(), SetRowBackColorI()` | (CSS `.IBColorAdded`) | ⚠️ | v8 에 **메서드가 없다.** css 파일에서 `.IBColorAdded` 클래스를 고친다(근거 v8 `migration/migration-04.md`). ★**실행 중 색을 바꾸던 코드는 그대로 옮길 수 없다** — CSS 는 정적이다 (`ROWBACKCOLOR_STATUS`) |
| `GetRowBackColorU(), SetRowBackColorU()` | (CSS `.IBColorChanged`) | ⚠️ | v8 에 **메서드가 없다.** css 파일에서 `.IBColorChanged` 클래스를 고친다(근거 v8 `migration/migration-04.md`). ★**실행 중 색을 바꾸던 코드는 그대로 옮길 수 없다** — CSS 는 정적이다 (`ROWBACKCOLOR_STATUS`) |
...
| `GetColWidth(), SetColWidth()` | `setAttribute(null, col, "Width", n)` / 읽기 `getAttribute(null, col, "Width")` | ⚠️ | ★**`setColWidth()` 로 그대로 옮기면 안 된다 — 이름은 같은데 의미가 다르다.** v7 `SetColWidth(Col, Width)` 는 너비를 **그 값으로 설정**하지만(`0`이면 가장 긴 내용에 맞춤), v8 `setColWidth(col, dx)` 는 **가감(delta)** 이다 — 양수면 늘고 음수면 준다(`funcs/core/set-col-width.md`). `SetColWidth("c",300)` → `setColWidth("c",300)` 은 **300px 가 되는 게 아니라 300px 만큼 넓어진다.** 정적 너비는 `setAttribute` 로 설정한다. 멀티레코드 시트의 더미 컬럼에는 `setColWidth` 가 동작하지 않는다 (`SETCOLWIDTH`) |
| `SetColProperty()` | `setAttribute()` **여러 번** | ⚠️ | ★**여러 속성을 한 번에 넘기는 기능이 v8 에 없다**(벤더 확인 2026-08-14). 속성별로 나눠 `setAttribute(row, col, attr, val, render)` 를 호출한다. ★**앞의 호출은 `render:0`, 마지막만 `render:1`** — `render` default 가 `1`(즉시 반영)이라 그대로 두면 속성 수만큼 다시 그린다. 예: `setAttribute(null,"qty","CanEdit",0,0); setAttribute(null,"qty","Align","Center",1);` · `row`=null 이면 열 속성, `col`=null 이면 행 속성 · `Visible` 처럼 전용 함수가 있으면 그것을 쓴다(`hideCol`/`showCol`) · `ComboText`/`ComboCode` 를 넘기던 호출은 `Enum`/`EnumKeys` 로 이름이 바뀐다 (`SETCOLPROPERTY`) |
...
| `ShowPivotTable()` | `makePivotTable()` | ✅ | **동일 기능 교체**(벤더 확인 2026-08-12). 둘 다 **데이터 시트에서 호출**하고 피벗 시트를 **스스로 만든다**. 다만 ①**피벗 시트 id 규칙이 바뀐다** — v7 `원본id + "_Pivot"`(접미사) → v8 `"pivotSheet_" + 원본id`(접두사). id 로 참조하는 코드·DOM 조회·문자열 조립을 함께 고칠 것 ②인자 구조가 다르다(`MAKEPIVOTTABLE_OPTIONS`) ③v8 피벗 시트는 `DataMerge` 미지원, 행 5,000·열 200 초과 시 성능 보장 안 됨 |
| `ShowProcessDlg()` | `showMessage()` | ✅ | |
| `ShowDebugMsg()` | (없음) | ❌ | 지원안함 |
| `ShowToolTip()` | 초기화 `Def.Col`(또는 `Def.Row`) 의 `Tip` | ⚠️ | ★**`showTip()` 으로 옮기면 안 된다 — 기능이 전혀 다르다.** v7 `ShowToolTip(use)` 는 *데이터 영역 **모든 셀**의 툴팁 사용 여부*이고, v8 `showTip(tip)` 은 *지금 마우스 커서 위치에 문자열 툴팁 하나를 즉시 띄우는* 명령형 메서드다(`onMouseMove` 등에서 호출, `hideTip` 으로 숨김). `ShowToolTip(1)` → `showTip(1)` 은 **오류 없이 엉뚱하게 동작한다.** ★v8 에 **시트 단위로 켜고 끄는 기능이 없다** — 초기화에서 `options.Def = { Col: { Tip: 1 } }` 로 모든 열의 기본값을 준다(벤더 확인 2026-08-14). ★**`Def.Col`·`Def.Row`(첫 글자 대문자)** — 소문자로 쓰면 v8 이 오류 없이 무시한다. `(col)Tip` default 는 `0`(사용 안 함)이라 `ShowToolTip(1)` 은 반드시 `Tip:1` 을 명시해야 살아나고, `ShowToolTip(0)` 은 기본값과 같아 옮기지 않아도 된다. 내용 변경은 `(col)TipValue`, 표시 방식은 `Cfg.StandardTip`, 지연은 `Cfg.TipStart` (`SHOWTOOLTIP`) |
...
| `CreatePivotTable()` | `makePivotTable()` (**1:1 아님**) | ⚠️ | ★**호출 주체가 뒤집힌다 — 자동 치환 금지**(`CREATEPIVOTTABLE`). v7 은 **미리 만들어 둔 피벗 시트에서** 부르고 데이터 시트를 인자로 받는다 — `피벗시트.CreatePivotTable({...}, 데이터시트)`. v8 은 반대로 **데이터 시트에서** 부르고 피벗 시트를 **스스로 만든다** — `데이터시트.makePivotTable(...)`. 따라서 ①받는 객체를 데이터 시트로 바꾸고 ②**미리 만들던 피벗 시트 생성 코드가 불필요**해지며 ③그 시트를 id 로 참조하던 곳을 정리한다(v8 id 는 `pivotSheet_원본id` 로 고정). 인자도 `Rows`·`Cols`·`Value`·`ValueType`(구분자 `|` 문자열) → `criterias`·`init`·`type` 구조로 다시 짠다. ★**v8 에 1:1 대응이 없다**(벤더 확인 2026-08-12) — `ShowPivotTable` 쪽이 v8 와 같은 동작이며, `CreatePivotTable` 도 결국 `makePivotTable` 로 다시 짜서 안내한다 |
...
| `ValidateFail()` | `onBeforeSave` 이벤트 return 값 | ⚠️ | v7 은 `OnValidation`·`OnEditValidation` **안에서만** 동작했다. v8 은 대응 메서드가 없어 **`onBeforeSave` 에서 `return true`** 로 중단한다 — 검증 로직의 **위치가 바뀐다.** 메시지(둘째 인자)도 함께 옮길 것 (`VALIDATEFAIL`) |
| `Version()` | `version()` | ✅ | |
