# GetRangeText ***(core method)***

> 인자로 설정한 범위 영역에 대한 CellText 값을 구분자로 연결한 문자열로 반환합니다.

### Syntax
```javascript
ObjId.GetRangeText(startRow, startCol, endRow, endCol, ColSeparator, RowSeparator);
```
...
var value = mySheet.GetRangeText(2, 1, 4, 5),
    rows  = [],
    cols  = [];
