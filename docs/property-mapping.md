...
| `AutoClearHeaderCheck` | ❌ 공개 API 로는 대응 없음 | ❌ | v7 은 **`RemoveAll` 호출·조회 시 헤더 전체체크를 자동 초기화**하는 속성이다. 이전에 `syncHeaderCheck()` 로 안내했으나 ①그 함수는 **v8 비공개 API**이고 ②의미도 다르다(데이터 행 값에 맞춰 헤더 체크를 동기화). `HeaderCheckMode` 도 "모든 행 vs 보이는 행만 체크" 라 다른 기능이다. **조회·삭제 후 직접 체크를 해제**하도록 코드로 처리한다 — 2026-08-04 정정 |
...
| IBSheet7 | IBSheet8 | 검증 | 비고 |
|---|---|---|---|
| `AcceptKeys` | `(Col)EditMask` | ✅ | 입력 허용 문자를 정규식으로 옮긴다(`props/cell/edit-mask.md`) |
...
| `ExceptKeys` | `(Col)EditMask` | ✅ | 입력 **제외** 문자는 정규식 부정 클래스로 옮긴다(`props/cell/edit-mask.md`) |
...
| `ItemText` | `(Col)Enum` | ✅ |  |
| `LevelSaveName` | `doSave`/`getSaveString` 의 `saveAttr` 인자 | ✅ | `saveAttr:"<tree컬럼명>Level"` 로 지정(`funcs/core/do-save.md`) |
...
| `NullInteger` | `Extend:IB_Preset.NullInteger` (또는 `Format:"#,###"`) | ⚠️ | ★**`CanEmpty:1` 을 함께 설정해야 한다**(벤더 확인). `CanEmpty:0` 이면 빈 값으로 둘 수 없어 **0 이 채워져 보인다**(`props/col/can-empty.md`) |
| `NullFloat` | `Extend:IB_Preset.NullFloat` (또는 `Format:"#,###.##"`) | ⚠️ | ★**`CanEmpty:1` 을 함께 설정해야 한다**(벤더 확인) — 위 `NullInteger` 와 같다 |
