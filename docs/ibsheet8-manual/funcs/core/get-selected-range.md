...
###
![선택영역얻기](/assets/imgs/getSelectedRanges.png "선택영역얻기")<br/>
...
|----------|-----|---|----|
|rowtype|`number`|<span class='optional'>선택</span>|`0`:구분없이 선택영역 모두 추출  (`default`)<br/>`1`:트리사용시 접힌 자식은 빼고 추출<br/>`2`:페이지 단위로 구분해서 추출|
|coltype|`number`|<span class='optional'>선택</span>|`0`:구분없이 모두 추출 (`default`)<br/>`1`:영역(Left/Center/Right)별로 구분해서 추출|
