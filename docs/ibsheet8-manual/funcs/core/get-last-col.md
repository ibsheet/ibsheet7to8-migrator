...
|----------|-----|---|----|
|section|`number`|<span class='optional'>선택</span>|틀고정 기준 영역 지정<br>`0`:좌측 (RightCols(우측), Cols(가운데) 정보가 없을 경우 `default`)<br>`1`:가운데 (RightCols(우측) 정보가 없을 경우 `default`)<br>`2`:우측 (`default`)|
