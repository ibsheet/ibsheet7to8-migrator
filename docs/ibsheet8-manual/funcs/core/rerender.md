...
|----------|-----|---|----|
|scroll|`boolean`|<span class='optional'>선택</span>|스크롤 바의 위치를 렌더링 이전과 동일하게 유지 여부<br>`0(false)`:스크롤 바의 위치 유지 안함<br>`1(true)`:스크롤 바의 위치를 렌더링 후에도 유지 (`default`)|
|sync|`boolean`|<span class='optional'>선택</span>|렌더링 작업을 동기로 처리 여부<br>`0(false)`:비동기 방식 (`default`)<br>`1(true)`:동기 방식|
