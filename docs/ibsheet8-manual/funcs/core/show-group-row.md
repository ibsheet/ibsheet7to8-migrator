...
|----------|-----|---|----|
|cols |`string` \| `object`|<span class='optional'>선택</span>|그룹핑 하고자 하는 컬럼. 단일로 설정할 경우 문자열로 설정하고, 여러개를 설정할 경우 배열로 설정합니다.|
|format |`string`|<span class='optional'>선택</span>|그룹 기준 컬럼의 데이터 포맷을 설정합니다. <br> - {%s} : 해당 셀의 값 <br> - - {%c} : 자식노드 개수|
