...
|callback|`function`|[showMessageTime (method)](/docs/funcs/core/show-message-time)으로 호출될때 설정된 `func`을 호출하는 콜백 메소드<br>buttons를 통해 설정한 버튼의 개수에 따라 좌측부터 순서대로 1,2,3...이 함수의 arguments로 리턴<br>(가령 버튼이 두 종류이면 클릭한 버튼에 따라 1 또는 2가 리턴 됩니다.)|
