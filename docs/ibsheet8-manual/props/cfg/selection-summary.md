...
|----------|-----|---|
|`Mode`|`string`|빈 셀을 계산에 포함할지 여부(`EmptyCell`), 삭제행을 계산에 포함할지 여부(`DelRow`), 인접하지 않은 모든 선택 영역을 계산할지 여부(`AllRange`)를 구분자 "\|"로 연결하여 설정 <br>(ex: "Mode":"EmptyCell\|DelRow\|AllRange") 기본값은 `""`(다 포함하지 않음)|
